  ...
  return (
    <div className="page-stack">
      ...
      <section className="access-grid">
        <Panel className="access-state-panel"><div className="panel-heading"><div><span className="panel-kicker">CURRENT PATH</span><h2>HTTP CONNECT</h2></div><StatusBadge status="enabled" /></div><div className="access-facts"><div><span>FQDN</span><strong className="mono">proxy.corp.internal</strong></div><div><span>Proxy port</span><strong>80</strong></div><div><span>HTTPS proxy listener</span><strong>Disabled</strong></div></div><div className="access-note"><ShieldCheck size={18} /><span>Destination HTTPS remains end-to-end inside the HTTP CONNECT tunnel.</span></div></Panel>
        <Panel><div className="panel-heading"><div><span className="panel-kicker">LINUX</span><h2>Setup script</h2></div><Laptop size={19} /></div><div className="script-actions"><Button variant="secondary" onClick={() => void copyScript()}><Clipboard size={16} /> {copied ? "Copied" : "Copy"}</Button><Button variant="primary" onClick={downloadScript}><Download size={16} /> Download</Button></div><pre className="script-view">{script.data}</pre></Panel>
      </section>
    </div>
  );
}
