...
import { Network, Pencil, Power } from "lucide-react";
...
export default function SitesPage() {
  ...
  return (
    <div className="page-stack page-fill list-page">
      <PageHeader eyebrow="OPERATE" title={t("Sites")} description={t("Regional edge sites and their enrolled nodes.")} />
      <Panel className="list-panel">
        <div className="table-toolbar"><div className="toolbar-title"><Network size={18} /><span>{t("{count} sites", { count: formatNumber(siteItems.length) })}</span></div><span className="toolbar-note">{t("Site configuration changes require a release before nodes apply them.")}</span></div>
        <div className="table-wrap table-scroll"><table><thead><tr><th>{t("Site")}</th><th>{t("Edge nodes")}</th><th>{t("Configuration revision")}</th><th>{t("Listener")}</th><th>{t("State")}</th><th aria-label={t("Actions")} /></tr></thead><tbody>{siteItems.map((site) => <tr key={site.id}><td><div className="cell-with-action"><strong>{t(site.name)}</strong><IconButton label={t("Rename site {name}", { name: site.name })} tooltip={false} onClick={() => { setEditSite(site); setEditName(site.name); rename.reset(); }}><Pencil size={14} /></IconButton></div><span className="cell-secondary">{site.slug}</span></td><td>{formatNumber(nodeCount.get(site.id) || 0)}</td><td>v{formatNumber(site.config_revision)}</td><td>HTTP :1080</td><td><StatusBadge status={site.shutdown ? "shutdown" : "active"} /></td><td><button className="row-icon-button" aria-label={t("{action} {name}", { action: t(site.shutdown ? "Restore" : "Shutdown"), name: t(site.name) })} title={t(site.shutdown ? "Restore" : "Shutdown")} onClick={() => setTarget(site)}><Power size={16} /></button></td></tr>)}</tbody></table></div>
      </Panel>
      <ConfirmDialog open={Boolean(target)} onOpenChange={(open) => !open && setTarget(null)} title={target?.shutdown ? "Restore site listener" : "Emergency shutdown"} description={target?.shutdown ? t("Restore proxy listener access for {name}. A release is still required for monitors to apply the state.", { name: t(target.name) }) : t("Block the proxy listener for {name}. This changes the desired configuration and requires a release to reach its edge nodes.", { name: t(target?.name || "") })} confirmLabel={target?.shutdown ? "Restore site" : "Shut down site"} danger={!target?.shutdown} busy={shutdown.isPending} onConfirm={() => target && shutdown.mutate({ site: target, enabled: !target.shutdown })} />
      <DetailDialog open={Boolean(editSite)} onOpenChange={(open) => { if (!open && !rename.isPending) setEditSite(null); }} title="Rename site" description={editSite ? t("Change the display name for {name}. The site slug, configuration, and node bindings remain unchanged.", { name: editSite.name }) : undefined} contentClassName="node-edit-dialog-content">
