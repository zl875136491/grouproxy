...
export default function ExceptionsPage() {
  ...
  return (
    <div className="page-stack page-fill list-page">
      <PageHeader eyebrow="POLICY" title="Policy exceptions" description="Temporary source access applies to all sites and expires automatically." actions={<Button variant="primary" onClick={() => setShowForm(true)}><CirclePlus size={16} /> {t("Add exception")}</Button>} />
      ...
      <DetailDialog open={showForm} onOpenChange={(open) => { if (!open && !create.isPending) setShowForm(false); }} title="Add exception" description="Temporarily allow a source CIDR or IP for every site." contentClassName="policy-form-dialog">
        <form className="policy-dialog-form" onSubmit={submit}>
          <label><span>{t("CIDR or IP")}</span><input autoFocus value={cidr} onChange={(event) => setCIDR(event.target.value)} placeholder="198.51.100.18/32" /></label>
          <label><span>{t("Owner")}</span><input value={owner} onChange={(event) => setOwner(event.target.value)} placeholder={t("Operator name")} /></label>
          <label><span>{t("Expires at")}</span><input type="datetime-local" value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)} /></label>
          <label><span>{t("Comment")}</span><input value={comment} onChange={(event) => setComment(event.target.value)} placeholder={t("Travel approval reference")} /></label>
          <div className="form-actions"><Button type="button" onClick={() => setShowForm(false)} disabled={create.isPending}>{t("Cancel")}</Button><Button variant="primary" type="submit" disabled={create.isPending}>{create.isPending ? t("Adding...") : t("Add exception")}</Button></div>
        </form>
      </DetailDialog>
