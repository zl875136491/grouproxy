...
export default function TasksPage() {
  ...
  return (
    <div className="page-stack">
      ...
      <Panel>
        {taskItems.length ? <div className="table-wrap"><table><thead><tr><th>Task</th><th>State</th><th>Progress</th><th>Retries</th><th>Lease</th><th>Updated</th><th aria-label="Actions" /></tr></thead><tbody>{taskItems.map((task) => { const canCancel = ["queued", "running"].includes(task.status); return <tr key={task.task_id}><td><strong>{label(task.task_type)}</strong><span className="cell-secondary mono">{task.task_id.slice(0, 14)}</span></td><td><StatusBadge status={task.status} /></td><td><div className="progress-cell"><div><span style={{ width: `${Math.max(0, Math.min(task.progress, 100))}%` }} /></div><strong>{task.progress}%</strong></div><span className="cell-secondary">{task.stage.replaceAll("_", " ")}</span></td><td>{task.retry_count} / {task.max_retries}</td><td>{task.locked_by || "-"}</td><td>{formatDate(task.finished_at || task.created_at)}</td><td>{canCancel ? <Button variant="ghost" size="sm" onClick={() => setCancelTarget(task)}><CircleStop size={14} /> Cancel</Button> : null}</td></tr>; })}</tbody></table></div> : <EmptyState title="No tasks recorded" />}
      </Panel>
      <ConfirmDialog open={Boolean(cancelTarget)} onOpenChange={(open) => !open && setCancelTarget(null)} title="Cancel task" description={`Request cancellation for ${cancelTarget ? label(cancelTarget.task_type) : "this task"}. Running work stops at its next safe checkpoint.`} confirmLabel="Cancel task" danger busy={cancel.isPending} onConfirm={() => cancel.mutate()} />
    </div>
  );
}
