...
export default function BlacklistPage() {
  ...
  return (
    <div className="page-stack page-fill list-page">
      <PageHeader eyebrow="POLICY" title="Destination deny" description="Blocked destination patterns are included in every generated bundle." actions={<Button variant="primary" onClick={() => setShowForm(true)}><CirclePlus size={16} /> {t("Add destination")}</Button>} />
      ...
      <DetailDialog open={showForm} onOpenChange={(open) => { if (!open && !create.isPending) setShowForm(false); }} title="Add destination" description="Block a domain, IP address, or CIDR in every generated bundle." contentClassName="policy-form-dialog">
        <form className="policy-dialog-form" onSubmit={submit}>
          <label><span>{t("Type")}</span><select value={kind} onChange={(event) => setKind(event.target.value as DestinationBlacklist["kind"])}><option value="domain">{t("Domain")}</option><option value="ip">{t("IP address")}</option><option value="cidr">CIDR</option></select></label>
          <label><span>{t("Pattern")}</span><input autoFocus value={pattern} onChange={(event) => setPattern(event.target.value)} placeholder={kind === "domain" ? "example.com" : kind === "ip" ? "203.0.113.10" : "203.0.113.0/24"} /></label>
          ...
          <div className="form-actions"><Button type="button" onClick={() => setShowForm(false)} disabled={create.isPending}>{t("Cancel")}</Button><Button variant="primary" type="submit" disabled={create.isPending}>{create.isPending ? t("Adding...") : t("Add destination")}</Button></div>
        </form>
      </DetailDialog>
