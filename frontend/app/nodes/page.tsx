...
export default function NodesPage() {
  ...
  useEffect(() => {
    const error = selectedNode?.last_error?.trim();
    if (!error) {
      nodeErrorKey.current = "";
      return;
    }
    const key = `${selectedNode?.agent_id}:${error}`;
    if (nodeErrorKey.current === key) return;
    nodeErrorKey.current = key;
    toast({
      title: t("Node reports an error"),
      description: t(error),
      variant: "destructive",
    });
  }, [selectedNode?.agent_id, selectedNode?.last_error, t, toast]);
