    ...
    <div className="page-stack">
      ...
      <Panel>
        <div className="table-toolbar">
          <div className="toolbar-title"><ServerCog size={18} /><span>{t("{count} nodes", { count: rows.length })}</span></div>
        ...
        <div className="table-wrap"><table><thead><tr><th>{t("Node")}</th><th>{t("Site")}</th><th>{t("Heartbeat")}</th><th>{t("Config")}</th><th>{t("Service")}</th><th>{t("Version")}</th><th aria-label={t("Actions")} /></tr></thead><tbody>{rows.map((node) => <tr key={node.id}><td><strong>{node.name}</strong><span className="cell-secondary mono">{node.agent_id}</span></td><td>{t(siteNames.get(node.site_id) || node.site_id)}</td><td><StatusBadge status={node.liveness_status} /></td><td><StatusBadge status={node.config_status} /></td><td><StatusBadge status={node.service_status} /></td><td><span className="version-pair">{node.applied_version} <span>/</span> {node.desired_version}</span></td><td><Button size="sm" variant="ghost" onClick={() => setSelectedNode(node)}>{t("Inspect")}</Button></td></tr>)}</tbody></table></div>
      </Panel>
