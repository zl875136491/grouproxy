  ...
  return (
    <div className="page-stack">
      ...
      <section className="subscription-layout">
        <Panel>
          <div className="panel-heading"><div><span className="panel-kicker">{t("UPSTREAMS")}</span><h2>{t("Sources")}</h2></div><span className="count-label">{sourceItems.length}</span></div>
      ...
      <Panel>
        <div className="panel-heading"><div><span className="panel-kicker">{t("IMMUTABLE HISTORY")}</span><h2>{t("Versions")}</h2></div><span className="count-label">{currentVersions.length}</span></div>
        {currentVersions.length ? <div className="table-wrap"><table><thead><tr><th>{t("Version")}</th><th>{t("Hash")}</th><th>{t("Format")}</th><th>{t("Nodes")}</th><th>{t("Parsed")}</th><th>{t("Fetched")}</th><th>{t("State")}</th></tr></thead><tbody>{currentVersions.map((version) => <tr className={currentVersion?.id === version.id ? "subscription-version-selected" : ""} key={version.id} onClick={() => setSelectedVersionId(version.id)}><td><strong>v{version.version}</strong><span className="cell-secondary">{t("{count} bytes", { count: version.size_bytes.toLocaleString(locale) })}</span></td><td className="mono" title={version.content_hash}>{shortHash(version.content_hash, 18)}</td><td><span className="type-tag">{version.format}</span></td><td>{version.node_count}</td><td><StatusBadge status={version.parse_ok ? "valid" : "failed"} /></td><td>{formatDate(version.fetched_at)}</td><td><StatusBadge status={version.published ? "published" : "ready"} /></td></tr>)}</tbody></table></div> : <EmptyState title="No versions for this source" detail="Refresh or import content to create an immutable version." />}
      </Panel>
      <Panel>
        <div className="panel-heading"><div><span className="panel-kicker">{t("SITE SELECTIONS")}</span><h2>{t("Active deployments")}</h2></div><span className="count-label">{activeBindings.length}</span></div>
      ...
      <ConfirmDialog open={Boolean(publishTarget)} onOpenChange={(open) => !open && setPublishTarget(null)} title="Publish subscription version" description={publishTarget ? t("Release {version} to {count} selected sites. Each edge node validates and acknowledges the new outbound configuration independently.", { version: versionLabel(publishTarget), count: selectedSiteIds.length }) : ""} confirmLabel="Publish version" busy={publish.isPending} onConfirm={() => publishTarget && publish.mutate(publishTarget)} />
...
function VersionDetail({
  ...
  return (
    <div className="subscription-detail">
      <div className="release-detail-heading"><div><span className="panel-kicker">{t("SELECTED VERSION")}</span><h2>{versionLabel(version)}</h2><p>{version.format} · {t("{count} bytes", { count: version.size_bytes.toLocaleString(locale) })} · {t("Fetched {date}", { date: formatDate(version.fetched_at) })}</p></div><StatusBadge status={version.parse_ok ? "valid" : "failed"} /></div>
      <div className="detail-summary-grid"><div><span>{t("Content hash")}</span><strong className="mono" title={version.content_hash}>{shortHash(version.content_hash, 18)}</strong></div><div><span>{t("Endpoint count")}</span><strong>{version.node_count}</strong></div><div><span>{t("Publication")}</span><StatusBadge status={version.published ? "published" : "ready"} /></div></div>
      {version.parse_ok ? <div className="subscription-site-selection"><div className="subscription-selection-heading"><div><span className="panel-kicker">{t("DEPLOY TARGETS")}</span><strong>{t("Sites")}</strong></div><span>{t("{count} selected", { count: selectedSiteIds.length })}</span></div><div className="subscription-site-checks">{siteItems.map((site) => { const deployable = deployableSiteIds.includes(site.id); return <label className={!deployable ? "subscription-site-disabled" : ""} key={site.id}><input type="checkbox" checked={selectedSiteIds.includes(site.id)} disabled={!deployable} onChange={() => onToggleSite(site.id)} /><span><strong>{t(site.name)}</strong><small>{deployable ? site.slug : t("No edge node")}</small></span></label>; })}</div><Button variant="primary" disabled={!selectedSiteIds.length} onClick={onPublish}><Play size={16} /> {t("Publish to selected sites")}</Button></div> : <div className="subscription-parse-error"><CircleAlert size={18} /><div><strong>{t("Version cannot be published")}</strong><span>{version.parse_error || t("Parsing did not produce usable outbounds.")}</span></div></div>}
    </div>
  );
}
